...
Diante desse cenário, este trabalho propõe o desenvolvimento de um sistema web de roteirização voltado ao cálculo da menor rota entre dois pontos em uma cidade. O sistema utilizará dados geográficos obtidos a partir do OpenStreetMap e aplicará o algoritmo de Dijkstra para determinar o caminho mais curto entre um ponto de origem e um ponto de destino informados pelo usuário.

A problemática abordada neste trabalho está diretamente relacionada à otimização de rotas, tema amplamente estudado na literatura por meio do *Vehicle Routing Problem* (VRP), que envolve a definição de rotas eficientes para atendimento de múltiplos pontos. Embora o problema tratado neste projeto seja uma versão simplificada, focada no cálculo do menor caminho entre dois pontos, ele compartilha fundamentos teóricos com esses estudos.

Assim, o objetivo geral deste estudo é desenvolver um sistema capaz de calcular rotas eficientes em ambientes urbanos por meio da aplicação de algoritmos de caminho mínimo sobre dados reais de redes viárias. Como objetivos específicos, destacam-se a modelagem de um banco de dados para armazenamento das informações do sistema, a obtenção e estruturação de dados cartográficos, a implementação do algoritmo de Dijkstra para cálculo de rotas e a construção de uma interface que permita ao usuário informar pontos de origem e destino e visualizar a rota calculada.
...
No contexto deste trabalho, o algoritmo de Dijkstra é utilizado para calcular a rota mais eficiente entre dois pontos em uma rede representada por um grafo ponderado. Dessa forma, ele permite determinar o caminho de menor custo entre origem e destino, contribuindo diretamente para a otimização de rotas em sistemas de entrega.
